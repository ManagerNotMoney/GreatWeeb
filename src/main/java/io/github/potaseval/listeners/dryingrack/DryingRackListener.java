...
public class DryingRackListener implements Listener {
    ...
    private void handleExplosion(Iterable<Block> blocks) {
        for (Block block : blocks) {
            if (block.getType() != Material.BARREL) continue;
            if (!(block.getState() instanceof TileState state)) continue;
            if (!state.getPersistentDataContainer().has(RACK_KEY, PersistentDataType.BOOLEAN)) continue;
            manager.breakRack(block.getLocation(), block.getWorld());
            state.getPersistentDataContainer().remove(RACK_KEY);
            state.update();
        }
    ...
    public void onPistonExtend(BlockPistonExtendEvent event) {
        if (containsRack(event.getBlocks())) {
            event.setCancelled(true);
        }
    }
    @EventHandler
    ...
    private boolean containsRack(Iterable<Block> blocks) {
        for (Block block : blocks) {
            if (block.getType() != Material.BARREL) continue;
            if (block.getState() instanceof TileState state) {
                if (state.getPersistentDataContainer().has(RACK_KEY, PersistentDataType.BOOLEAN)) {
                    return true;
                }
            }
        }
